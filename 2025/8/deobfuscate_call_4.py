...
pattern = [
    ...
    {"inst": idaapi.NN_sub, "op1": idaapi.o_reg, "op2": idaapi.o_imm},
    {"inst": idaapi.NN_lea, "op1": idaapi.o_reg, "op2": idaapi.o_displ},
    ...
    while ea < end_ea:
        ...
        for i, pat in enumerate(pattern):
            if instn.itype != pat['inst'] or instn.Op1.type != pat['op1'] or instn.Op2.type != pat['op2']:
                is_match=False
                break
            if instn.itype not in [idaapi.NN_callni, idaapi.NN_sub, idaapi.NN_lea]:
...
def fix_bytes(matches):
    ...
    for i, match in enumerate(matches):
        ...
        new_value = (mem_val + imm_val) & ((1<<64)-1)
